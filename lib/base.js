...
define(
  ...
  function(utils, registry, debug) {
    ...
    function proxyEventTo(targetEvent) {
      function proxy(e, data) {
        $(e.target).trigger(targetEvent, data);
      }
      proxy.targetEvent = targetEvent
      return proxy;
    }

    function unProxyEventTo(element, sourceEvent, targetEvent) {
      var instance = this;
      registry.findInstanceInfo(this).events.forEach(function (eventData) {
        var eventDataProxyTargetEvent = '';
        try {
          eventDataProxyTargetEvent = eventData.callback.target.targetEvent;
        } catch (e) {}
        console.log('eventDataProxyTargetEvent', eventDataProxyTargetEvent);
        if (eventData.type === sourceEvent && targetEvent === eventDataProxyTargetEvent) {
          console.log('== OFFING ========================');
          console.log('element', element);
          console.log('sourceEvent', sourceEvent);
          console.log('eventData.callback', eventData.callback);
          instance.off(element, sourceEvent, eventData.callback);
        }
      });
    }
    ...
    function withBase() {
       ...
       * this.off(document, 'click', this.handleClick)
       * this.off('click', 'uiWasClicked')
       */
      this.off = function() {
        ...
        var argsIsElementProxy = (
          arguments.length === 3 &&
          typeof arguments[1] === 'string' &&
          typeof arguments[2] === 'string'
        );

        // If we're proxying, get out early
        if (argsIsProxy || argsIsElementProxy) {
          var proxyElement = (argsIsElementProxy ? arguments[0] : this.$node);
          var proxyEvents = (
            argsIsElementProxy ?
              [ arguments[1], arguments[2] ] :
              [ arguments[0], arguments[1] ]
          );
          unProxyEventTo.apply(this, [proxyElement].concat(proxyEvents));
          return proxyElement;
        }
